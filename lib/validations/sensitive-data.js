...
module.exports = {
    ...
    configurator (currentVal) {
        function configureIgnores () {
            ...
            return confirm('Is there any files that you want to exclude from check?', false)
                .then(yes => yes ? inputList('List files you want to exclude (comma-separated, you can use glob patterns)', ignore) : true)
                .then(answer => Array.isArray(answer) ? { ignore: answer } : answer);
        }

        return confirm('Would you like to verify that there is no sensitive data in your working tree?', !!currentVal)
